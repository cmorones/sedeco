<?php
/* @var $this Ap8Ind11Controller */
/* @var $model Ap8Ind11 */
/* @var $form CActiveForm */
?>

<div class="form">

<?php $form=$this->beginWidget('CActiveForm', array(
	'id'=>'ap8-ind11-form',
	// Please note: When you enable ajax validation, make sure the corresponding
	// controller action is handling ajax validation correctly.
	// There is a call to performAjaxValidation() commented in generated controller code.
	// See class documentation of CActiveForm for details on this.
	'enableAjaxValidation'=>false,
)); //para el id de periodo y el last insert
$id = Yii::app()->request->getQuery('id');

$sql_lid="SELECT MAX(id) as id  FROM ap8Ind11";
$lid = Yii::app()->db->createCommand($sql_lid)->queryRow();
$lastid=$lid['id']+1;
?>

	<p class="note">Fields with <span class="required">*</span> are required.</p>

	<?php echo $form->errorSummary($model); ?>

	

	<div class="row">
		<?php echo $form->labelEx($model,'anio'); ?>
		<?php echo $form->textField($model,'anio'); ?>
		<?php echo $form->error($model,'anio'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'mes'); ?>
		<?php echo $form->textField($model,'mes'); ?>
		<?php echo $form->error($model,'mes'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'p_ocupadas_n_ma'); ?>
		<?php echo $form->textField($model,'p_ocupadas_n_ma'); ?>
		<?php echo $form->error($model,'p_ocupadas_n_ma'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'p_ocupadas_df_ma'); ?>
		<?php echo $form->textField($model,'p_ocupadas_df_ma'); ?>
		<?php echo $form->error($model,'p_ocupadas_df_ma'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'p_ocupadas_n_me'); ?>
		<?php echo $form->textField($model,'p_ocupadas_n_me'); ?>
		<?php echo $form->error($model,'p_ocupadas_n_me'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'p_ocupadas_df_me'); ?>
		<?php echo $form->textField($model,'p_ocupadas_df_me'); ?>
		<?php echo $form->error($model,'p_ocupadas_df_me'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'remuneraciones_n_ma'); ?>
		<?php echo $form->textField($model,'remuneraciones_n_ma'); ?>
		<?php echo $form->error($model,'remuneraciones_n_ma'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'remuneraciones_df_ma'); ?>
		<?php echo $form->textField($model,'remuneraciones_df_ma'); ?>
		<?php echo $form->error($model,'remuneraciones_df_ma'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'remuneraciones_n_me'); ?>
		<?php echo $form->textField($model,'remuneraciones_n_me'); ?>
		<?php echo $form->error($model,'remuneraciones_n_me'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'remuneraciones_df_me'); ?>
		<?php echo $form->textField($model,'remuneraciones_df_me'); ?>
		<?php echo $form->error($model,'remuneraciones_df_me'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'remuneraciones_pp_n_ma'); ?>
		<?php echo $form->textField($model,'remuneraciones_pp_n_ma'); ?>
		<?php echo $form->error($model,'remuneraciones_pp_n_ma'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'remuneraciones_pp_df_ma'); ?>
		<?php echo $form->textField($model,'remuneraciones_pp_df_ma'); ?>
		<?php echo $form->error($model,'remuneraciones_pp_df_ma'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'remuneraciones_pp_n_me'); ?>
		<?php echo $form->textField($model,'remuneraciones_pp_n_me'); ?>
		<?php echo $form->error($model,'remuneraciones_pp_n_me'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'remuneraciones_pp_df_me'); ?>
		<?php echo $form->textField($model,'remuneraciones_pp_df_me'); ?>
		<?php echo $form->error($model,'remuneraciones_pp_df_me'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'ingreso_n_ma'); ?>
		<?php echo $form->textField($model,'ingreso_n_ma'); ?>
		<?php echo $form->error($model,'ingreso_n_ma'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'ingreso_df_ma'); ?>
		<?php echo $form->textField($model,'ingreso_df_ma'); ?>
		<?php echo $form->error($model,'ingreso_df_ma'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'ingreso_n_me'); ?>
		<?php echo $form->textField($model,'ingreso_n_me'); ?>
		<?php echo $form->error($model,'ingreso_n_me'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'ingreso_df_me'); ?>
		<?php echo $form->textField($model,'ingreso_df_me'); ?>
		<?php echo $form->error($model,'ingreso_df_me'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'compras_n_ma'); ?>
		<?php echo $form->textField($model,'compras_n_ma'); ?>
		<?php echo $form->error($model,'compras_n_ma'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'compras_df_ma'); ?>
		<?php echo $form->textField($model,'compras_df_ma'); ?>
		<?php echo $form->error($model,'compras_df_ma'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'compras_n_me'); ?>
		<?php echo $form->textField($model,'compras_n_me'); ?>
		<?php echo $form->error($model,'compras_n_me'); ?>
	</div>

	<div class="row">
		<?php echo $form->labelEx($model,'compras_df_me'); ?>
		<?php echo $form->textField($model,'compras_df_me'); ?>
		<?php echo $form->error($model,'compras_df_me'); ?>
	</div>

	<div class="row">
		
		<?php echo $form->hiddenField($model,'id', array('value'=>$lastid)); ?>
		<?php    $cadena=stripos($_SERVER['REQUEST_URI'], 'update');    if($cadena<1){    echo $form->hiddenField($model,'periodo_id', array('value'=>$id));   }else{  echo $form->hiddenField($model,'periodo_id');   }    ?>
		<?php echo $form->hiddenField($model,'fecha_reg', array('value'=>date("Y\-n\-j H:i:s"))); ?>
		<?php echo $form->hiddenField($model,'user_reg', array('value'=>Yii::app()->user->id)); ?>
		
	</div>
	<div class="row buttons">
		<?php echo CHtml::submitButton($model->isNewRecord ? 'Agregar' : 'Guardar'); ?>
	</div>

<?php $this->endWidget(); ?>

</div><!-- form -->