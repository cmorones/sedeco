...
class Ap1Ind4aHistController extends Controller
{
	/**
	  ...
	  public function actionPrevio($id)
    {
        ...
        $model=Ap1Ind4aHist::model()->findByPk($id);
       // $baseUrl = YiiBase::getPathOfAlias("webroot");
        $url = "http://localhost/recm/index.php/api2/ap1Ind4as?id=$id";
        //$url = $baseUrl;
        $data = file_get_contents($url);
        $model= CJSON::decode($data);

         $url2 = "http://localhost/recm/index.php/api2/ap1Ind4ad?id=$id";
        //$url = $baseUrl;
	 ...
	 */
	   public function actionIndex() {
        ...
        $item_count = Ap1Ind4aHist::model()->count($criteria);

        $pages = new CPagination($item_count);
        $pages->setPageSize(10);
        $pages->applyLimit($criteria);  // the trick is here!

        $model= Ap1Ind4aHist::model()->findAll($criteria);
