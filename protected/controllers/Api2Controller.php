...
public function actionMenusite2(){
...
$resultado = Menus::model()->findAll((array(
	...
	foreach ($resultado as $key => $row) {
		...
		if( !isset($json[$row["label"]])): 
			$json[$row["label"]] = array(
			...
			/*$json[$row["label"]]['items']['Resumen'] = array(
		...
		$resultado2 = Menus::model()->findAll((array(
			...
			foreach ($resultado2 as $key2 => $row2) {
				...
				if( !isset($json['items'][$row2["label"]])):
				...
				$json[$row["label"]]['items'][$row2["label"]] = array(
	  ...
	  /*  $resultado3 = Menus::model()->findAll((array(
			...
			foreach ($resultado3 as $key3 => $row3) {
				...
				if( !isset($json['items'][$row2["label"]]['items'][$row3["label"]])): 
				$json[$row["label"]]['items'][$row2["label"]]['items'][$row3["label"]] = array(
			...
			foreach ($resultado as $key => $row) {
				...
				$entidad = Yii::app()->db->createCommand($sql)->queryRow();
						...
						if( !isset($json["informe"]['estado'][$entidad["nombre"]]) ): 
									$json["informe"]['anio']=array('valor'=>$row["anio"]);
