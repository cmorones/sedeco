...
class Ap5Ind23 extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id', 'required'),
			array('id, delegacion, rubro, anio, user_reg, user_mod, periodo_id', 'numerical', 'integerOnly'=>true),
			array('unidades, porcentual', 'numerical'),
			array('fecha_reg, fecha_mod', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, delegacion, unidades, porcentual, rubro, anio, fecha_reg, fecha_mod, user_reg, user_mod, periodo_id', 'safe', 'on'=>'search'),
		);
	}
