...
class Ap1Ind72 extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id', 'required'),
			array('id, rubro, anio, mes, columna, user_reg, user_mod, periodo_id', 'numerical', 'integerOnly'=>true),
			array('valor', 'numerical'),
			array('fecha_reg, fecha_mod', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, rubro, valor, anio, mes, columna, fecha_reg, fecha_mod, user_reg, user_mod, periodo_id', 'safe', 'on'=>'search'),
		);
	}
