...
class Ap1Ind4c extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('periodo_id, anio, id_trimestre, user_reg, user_mod', 'numerical', 'integerOnly'=>true),
			array('pib, igae, itaee', 'numerical'),
			array('fecha_reg, fecha_mod', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, periodo_id, anio, id_trimestre, pib, igae, itaee, fecha_reg, fecha_mod, user_reg, user_mod', 'safe', 'on'=>'search'),
		);
	}
